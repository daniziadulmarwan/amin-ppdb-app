...
function GenderChart() {
    fetch("/Chart")
        ...
        .then((data) => {
            ...
            let series = [
                Math.round((man / data.gender.length) * 100),
                Math.round((woman / data.gender.length) * 100),
            ];
            var options = {
                chart: {
                    height: 370,
...
function FatherJobChart() {
    fetch("/Chart")
        .then((res) => res.json())
        .then((data) => {
            let noJob = data.fatherJob.filter(
                (w) => w.pekerjaan_ayah === 1
            ).length;
            let buruh = data.fatherJob.filter(
                (w) => w.pekerjaan_ayah === 2
            ).length;
            let dokter = data.fatherJob.filter(
                (w) => w.pekerjaan_ayah === 3
            ).length;
            let dosen = data.fatherJob.filter(
                (w) => w.pekerjaan_ayah === 4
            ).length;
            let sailor = data.fatherJob.filter(
                (w) => w.pekerjaan_ayah === 5
            ).length;
            let pedagang = data.fatherJob.filter(
                (w) => w.pekerjaan_ayah === 6
            ).length;
            let swasta = data.fatherJob.filter(
                (w) => w.pekerjaan_ayah === 7
            ).length;
            let hukum = data.fatherJob.filter(
                (w) => w.pekerjaan_ayah === 8
            ).length;
            let farmer = data.fatherJob.filter(
                (w) => w.pekerjaan_ayah === 9
            ).length;
            let pns = data.fatherJob.filter(
                (w) => w.pekerjaan_ayah === 10
            ).length;
...
function FatherSalaryChart() {
    fetch("/Chart")
        .then((res) => res.json())
        .then((data) => {
            let one = data.fatherSalary.filter(
                (w) => w.penghasilan_ayah === 1
            ).length;
            let two = data.fatherSalary.filter(
                (w) => w.penghasilan_ayah === 2
            ).length;
            let three = data.fatherSalary.filter(
                (w) => w.penghasilan_ayah === 3
            ).length;
            let four = data.fatherSalary.filter(
                (w) => w.penghasilan_ayah === 4
            ).length;
            let five = data.fatherSalary.filter(
                (w) => w.penghasilan_ayah === 5
            ).length;
            let six = data.fatherSalary.filter(
                (w) => w.penghasilan_ayah === 6
            ).length;
            let seven = data.fatherSalary.filter(
                (w) => w.penghasilan_ayah === 7
            ).length;
